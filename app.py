# Notes of interest.
#   https://github.com/MalphasWats/flask-blueprint-loader/blob/master/dashboard/dashboard.py
...
logclass.logger.info('Modula starting!')
...
# Demo of request args.
@app.route('/demorequest',methods=['GET', 'POST'])
def demorequest():
    if request.method == 'GET':
        logclass.logger.info('Request to root')
        return "remote_addr -> %s \nrequest_headers ->\n%s \ncontent_type -> %s\nis_secure -> %s\nquery_string -> %s\n" %(  str(request.remote_addr),  str(request.headers), str(request.content_type) , str(request.is_secure), str(request.query_string)  )
